...
local githubRawBase = 'https://raw.githubusercontent.com/Sadicius/rsg-versioncheckers/main/'
...
local function CheckVersion()
    ...
    -- Fallback timeout to avoid startup blocking if GitHub is unreachable
    SetTimeout(5000, function()
        if not completed then
            printLog('warning', 'Version check timeout after 5s; continuing startup.')
        end
    end)
    ...
    PerformHttpRequest(versionUrl, function(statusCode, remoteVersion, headers)
        ...
        if isVersionOutdated(currentVersion, remoteVersion) then
            ...
            printLog('error', 'Download from: https://github.com/Sadicius/'..GetCurrentResourceName()..'')
        else
